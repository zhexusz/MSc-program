...
'''
Add：按元素添加输入的层。
BatchNormalization：标准化前一层激活的层，有助于梯度流并提高训练稳定性。
Concatenate：沿指定轴连接多个输入的层。
Conv2D：对输入数据应用指定数量的滤波器的卷积层，通常用于图像处理任务。
LeakyReLU：通过允许小的负值在网络中引入非线性的激活函数。
MaxPooling2D：对输入数据执行最大池化的层，减少空间维度。
UpSampling2D：对输入数据执行上采样的层，增加空间维度。
ZeroPadding2D：用零填充输入数据的空间维度的层。
'''
